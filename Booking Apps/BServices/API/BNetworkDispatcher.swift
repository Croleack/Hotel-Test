...
extension INetworkDispatcher {
    ...
    func fetchData<T: Decodable>(
	   ...
	   completion: @escaping (Result<T, BNetworkError>) -> Void
    ) {
	   ...
	   let request = makeRequest(url: url, apiManager: apiManager)
...
final class BNetworkDispatcher: INetworkDispatcher {

    func fetchHotels(
	   completion: @escaping (Result<BHotelData, BNetworkError>) -> Void
    ) {
	   let apiRouter: BNetworkManager = .hotel
	   fetchData(apiManager: apiRouter, completion: completion)
    }
    
    func fetchRooms(
	   completion: @escaping (Result<BRoomData, BNetworkError>) -> Void
    ) {
	   let apiRouter: BNetworkManager = .rooms
	   fetchData(apiManager: apiRouter, completion: completion)
    }
    
    func fetchBooking(
	   completion: @escaping (Result<BookingData, BNetworkError>) -> Void
    ) {
	   let apiRouter: BNetworkManager = .booking
	   fetchData(apiManager: apiRouter, completion: completion)
    }
}
